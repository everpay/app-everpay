<?=$this->load->view(branded_view('common/header-home'));?>
<style>
.center-heading h3 {
    font-size: 1.8em;
    font-weight: 300;
    letter-spacing: normal;
    line-height: 36px;
    margin-bottom: 33px;
	margin-top:33px
}

.gm-style {
    font-family: Roboto,Arial,sans-serif;
    font-size: 11px;
    font-weight: 400;
    text-decoration: none;
}


.form-contact form label {
    font-size: 15px;
    font-weight: 400;
    color: #fff;
}

@media screen and (min-width: 780px)
.wrapper {
    width: 90%;
    padding-left: 0;
    padding-right: 0;
    max-width: 1120px;
    margin: 0 auto;
}
.faqIntro {
    float: none;
    margin: 0 auto;
    padding: 40px 0;
}
@media screen and (min-width: 1024px)
.wrapper .box {
    width: 80%;
    float: none;
    margin: 0 auto;
}

.box {
    padding-left: 0;
    padding-right: 0;
    padding: 0 5%;
}

.faqIntro .title, .faqIntro .viewMore {
    text-align: center;
}

.faqIntro ol {
    padding: 30px 0;
 min-height: 1%;
}
ol {
    display: block;
    list-style-type: none;
    -webkit-margin-before: 1em;
    -webkit-margin-after: 1em;
    -webkit-margin-start: 0px;
    -webkit-margin-end: 0px;
    -webkit-padding-start: 40px;
}

.faqIntro .viewMore {
    clear: both;
}
.faqIntro .title, .faqIntro .viewMore {
    text-align: center;
}

@media screen and (min-width: 780px)
.view-features .faqIntro .feature {
    min-height: 150px;
}
.faqIntro .feature {
    position: relative;
}

.feature {
    padding: 15px 0;
}

.faqIntro span {
    text-align: center;
    line-height: 24px;
    width: 26px;
    height: 26px;
    border: 1px solid #7b7b7b;
    border-radius: 24px;
    position: absolute;
    top: 13px;
    left: 0;
    z-index: 1;
}
.faqIntro em, .faqIntro span {
    display: block;
}

.faqIntro li h4 {
    padding: 0 40px 10px;
}

.faqIntro .feature p {
    padding-right: 40px;
}
.faqIntro li p {
    padding-left: 30px;
}

.col-6 {
    width: 50%;
}

#custom_map {
    height: 400px;
    display: block;
}

address {
    font-weight: 700;
	font-size: 16px;
    line-height: 1.42857143;
}

.contact-questions ul, li {
    list-style: none;
}

.contact-questions li:first-child {
    margin-top: 0;
}
.contact-questions li:first-child {
    margin-top: 0;
}
.contact-questions li {
    position: relative;
}

.contact-questions li>a:hover {
    background: rgba(239,242,244,0.3);
    color: #4c555a;
}
.contact-questions li>a:hover {
    background: rgba(239,242,244,0.3);
    color: #4c555a;
}
.contact-questions li>a {
    display: block;
    padding: 30px 70px 30px 125px;
    color: #6f7c82;
    transition: background-color 150ms;
}
.footer-btm{
    margin-top: 10px;
    padding: 20px;
    font-family: "proxima-nova", Arial, sans-serif;
}
</style>


        <!--parallax background no slider-->
        <!--rev slider start-->

        <div class="fullwidthbanner" id="home">
            <div class="ken-burns-banner">
                <ul>    <!-- SLIDE  -->
                    <li class="dark" data-transition="fade" data-slotamount="1" data-masterspeed="1500" data-thumb="" data-delay="10000"  data-saveperformance="off" data-title="">
                        <!-- MAIN IMAGE -->
                        <img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1443806954/landing-page-background-95d0cfcbfa7a88691182751427c8f404_ifqebe.jpg"  alt=""  data-bgposition="left center" data-kenburns="on" data-duration="10000" data-ease="Linear.easeNone" data-bgfit="130" data-bgfitend="100" data-bgpositionend="right center">
                        <!-- LAYERS -->
                        <div class="caption customin customout tp-resizeme ken-burns-cap  font-alt" 
                             data-x="center" 
                             data-hoffset="0" 
                             data-y="center" 
                             data-voffset="-100" 
                             data-customin="x:-50;y:-300;z:0;rotationX:0;rotationY:0;rotationZ:0;scaleX:0.5;scaleY:0.5;skewX:0;skewY:0;opacity:0;transformPerspective:0;transformOrigin:50% 50%;" 
                             data-customout="x:0;y:0;z:0;rotationX:0;rotationY:0;rotationZ:0;scaleX:0.75;scaleY:0.75;skewX:0;skewY:0;opacity:0;transformPerspective:600;transformOrigin:50% 50%;" 

                             data-speed="800" 
                             data-start="1000" 
                             data-startslide="1" 

                             data-easing="Power4.easeOut" 
                             data-endspeed="500" 
                             data-endeasing="Power4.easeIn">

                           Innovative Tools To Grow Your Business

                        </div>

                        <div class="caption customin customout tp-resizeme Ken-burns-heading font-alt" 
                             data-x="center" 
                             data-hoffset="0" 
                             data-y="center" 
                             data-voffset="0" 
                             data-customin="x:0;y:0;z:0;rotationX:90;rotationY:0;rotationZ:0;scaleX:1;scaleY:1;skewX:0;skewY:0;opacity:0;transformPerspective:200;transformOrigin:50% 0%;" 
                             data-customout="x:0;y:0;z:0;rotationX:0;rotationY:0;rotationZ:0;scaleX:0.75;scaleY:0.75;skewX:0;skewY:0;opacity:0;transformPerspective:600;transformOrigin:50% 50%;" 
                             data-speed="800" 
                             data-start="1300" 
                             data-startslide="1" 

                             data-easing="Power4.easeOut" 
                             data-endspeed="500" 
                             data-endeasing="Power4.easeIn">

                           Simple Global Payments

                        </div>


                        <div class="caption customin customout tp-resizeme" 
                             data-x="center" 
                             data-hoffset="0" 
                             data-y="center" 
                             data-voffset="120" 
                             data-customin="x:50;y:150;z:0;rotationX:0;rotationY:0;rotationZ:0;scaleX:0.5;scaleY:0.5;skewX:0;skewY:0;opacity:0;transformPerspective:0;transformOrigin:50% 50%;" 
                             data-customout="x:0;y:0;z:0;rotationX:0;rotationY:0;rotationZ:0;scaleX:0.75;scaleY:0.75;skewX:0;skewY:0;opacity:0;transformPerspective:600;transformOrigin:50% 50%;" 
                             data-speed="800" 
                             data-start="1500" 
                             data-startslide="1" 

                             data-easing="Power4.easeOut" 
                             data-endspeed="500" 
                             data-endeasing="Power4.easeIn">
                            <div class="rev-buttons scroll-to">

                      <a href="<?=$this->config->item('signup_url');?>" class="btn btn-lg border-white">Get Started</a>

                            </div>

                        </div>
                    </li>
					
					<!-- SLIDE -->
                    <li class="dark" data-transition="fade" data-slotamount="5" data-thumb="" data-masterspeed="1000" data-title="">
                        <!-- MAIN IMAGE -->
                        <img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1444311681/bg-vid_rnwske.png"  alt="darkblurbg"  data-bgfit="cover" data-bgposition="left top" data-bgrepeat="no-repeat">
                        <div class="tp-caption lft"
                             data-x="105" 
                             data-y="190"
                             data-speed="900"
                             data-start="800" 
                             data-easing="Sine.easeOut">
                            <img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1444320591/everpay-macbook_epowgv.png" alt="" />
                        </div>
                        <div class="tp-caption lfr"
                             data-x="335"
                             data-y="220"
                             data-speed="900" 
                             data-start="1500"
                             data-easing="Sine.easeOut">
                            <img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1444320591/everpay-ipad_jhns0k.png" alt="" />
                        </div>
                        <div class="tp-caption lfl" 
                             data-x="328"
                             data-y="329"
                             data-speed="900"
                             data-start="1800"
                             data-easing="Sine.easeOut">
                            <img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/c_scale,h_215/v1444322020/everpay-card-reader_o7buj6.png" alt="" />
                        </div>
                        <div class="caption sfr modern_big_redbg" 
                             data-x="685"
                             data-y="210"
                             data-speed="400"
                             data-start="2500"
                             data-easing="Sine.easeOut">
                            Ready Made For Mobile
                        </div>
                        <div class="caption sfr modern_big_redbg"
                             data-x="685" 
                             data-y="280"
                             data-speed="400"
                             data-start="2900"
                             data-easing="Sine.easeOut">
                            International Currencies
                        </div>
                        <div class="caption sfr modern_big_redbg"
                             data-x="655" data-y="345"
                             data-speed="400"
                             data-start="3300" 
                             data-easing="Sine.easeOut">
                            Ideal For Any Business Type
                        </div>
                        <div class="caption sfr rev-buttons tp-resizeme " 
                             data-x="780"
                             data-y="430" 
                             data-speed="400" 
                             data-start="3700" 
                             data-easing="Sine.easeOut">
                           <a href="<?=$this->config->item('signup_url');?>" class="btn btn-lg border-white">Get Started</a>
                        </div>


                    </li>

					
                    <!-- SLIDE  -->
                    <li class="dark" data-transition="slideleft" data-slotamount="1" data-masterspeed="1500" data-thumb="" data-delay="10000"  data-saveperformance="off"  data-title="">
                        <!-- MAIN IMAGE -->
                        <img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1442882923/hero-homepage-md_kkmwlh.jpg"  alt="Mobile Payments"  data-bgposition="center bottom" data-kenburns="on" data-duration="10000" data-ease="Linear.easeNone" data-bgfit="100" data-bgfitend="120" data-bgpositionend="center top">
                        <!-- LAYERS -->


                        <div class="caption customin customout tp-resizeme ken-burns-cap  font-alt" 
                             data-x="center" 
                             data-hoffset="0" 
                             data-y="center" 
                             data-voffset="-100" 
                             data-customin="x:-50;y:-300;z:0;rotationX:0;rotationY:0;rotationZ:0;scaleX:0.5;scaleY:0.5;skewX:0;skewY:0;opacity:0;transformPerspective:0;transformOrigin:50% 50%;" 
                             data-customout="x:0;y:0;z:0;rotationX:0;rotationY:0;rotationZ:0;scaleX:0.75;scaleY:0.75;skewX:0;skewY:0;opacity:0;transformPerspective:600;transformOrigin:50% 50%;" 

                             data-speed="800" 
                             data-start="800" 
                             data-startslide="1" 

                             data-easing="Power4.easeOut" 
                             data-endspeed="500" 
                             data-endeasing="Power4.easeIn">

                            A Frictionless Way To Conduct Global Commerce

                        </div>

                        <div class="caption customin customout tp-resizeme Ken-burns-heading font-alt" 
                             data-x="center" 
                             data-hoffset="0" 
                             data-y="center" 
                             data-voffset="-14" 
                             data-customin="x:0;y:0;z:0;rotationX:90;rotationY:0;rotationZ:0;scaleX:1;scaleY:1;skewX:0;skewY:0;opacity:0;transformPerspective:200;transformOrigin:50% 0%;" 
                             data-customout="x:0;y:0;z:0;rotationX:0;rotationY:0;rotationZ:0;scaleX:0.75;scaleY:0.75;skewX:0;skewY:0;opacity:0;transformPerspective:600;transformOrigin:50% 50%;" 
                             data-speed="800" 
                             data-start="1200" 
                             data-startslide="1" 

                             data-easing="Power4.easeOut" 
                             data-endspeed="500" 
                             data-endeasing="Power4.easeIn">

                          One Payment Portal

                        </div>


                        <div class="caption customin customout tp-resizeme" 
                             data-x="center" 
                             data-hoffset="0" 
                             data-y="center" 
                             data-voffset="83" 
                             data-customin="x:50;y:150;z:0;rotationX:0;rotationY:0;rotationZ:0;scaleX:0.5;scaleY:0.5;skewX:0;skewY:0;opacity:0;transformPerspective:0;transformOrigin:50% 50%;" 
                             data-customout="x:0;y:0;z:0;rotationX:0;rotationY:0;rotationZ:0;scaleX:0.75;scaleY:0.75;skewX:0;skewY:0;opacity:0;transformPerspective:600;transformOrigin:50% 50%;" 
                             data-speed="800" 
                             data-start="1500" 
                             data-startslide="1" 

                             data-easing="Power4.easeOut" 
                             data-endspeed="500" 
                             data-endeasing="Power4.easeIn">

                            <div class="rev-buttons scroll-to">

                                <a href="<?=$this->config->item('signup_url');?>" class="btn btn-lg border-white">Get Started</a>

                            </div>

                        </div>
                    </li>
					
                </ul>
            </div>
        </div><!--full width banner-->

        <!--navigation -->
        <!-- Static navbar -->
	<div class="navbar navbar-inverse navbar-static-top yamm sticky" role="navigation" style="position: fixed; top: 0px;">
            <div class="container">
                <div class="navbar-header">
                    <button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
                        <span class="sr-only">Toggle navigation</span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                    </button>
                    <a class="navbar-brand" href="<?=site_url('/');?>"><img src="https://everpayinc.com/assets/img/logo/white.png" width="100" alt=""></a>
                </div>
                <div class="navbar-collapse collapse">
                    <ul class="nav navbar-nav navbar-right scroll-to">
                        <li class="active"> <a href="<?=site_url('/#home');?>"><i class="fa fa-home fa-lg home-link"></i></a></li>
                        <li class=""> <a href="<?=site_url('/#features');?>">Features </a></li>
                        <li class=""> <a href="<?=site_url('/#how-it-works');?>">How It Works </a></li>                     
					    <li class=""> <a href="<?=site_url('/#price');?>">Pricing</a></li>						
			            <li class=""> <a href="<?=site_url('/#our-team');?>">Our Team </a></li> 
  			            <li class=""> <a href="<?=$this->config->item('contact_url');?>">Contact</a></li>
              			<li class="dropdown"><!--menu More li end here-->
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">More <i class="fa fa-angle-down"></i></a>
                            <ul class="dropdown-menu" role="menu">  
							   <li><a href="<?=$this->config->item('blog_url');?>">Our Blog</a></li>    
                               <li><a href="<?=$this->config->item('partners_url');?>">Partnerships</a></li>
                               <li><a href="<?=$this->config->item('developers_url');?>">Developers</a></li>
                               <li><a href="<?=$this->config->item('docs_url');?>">Documentation</a></li>
                               <li><a href="//github.com/everpay/">Code Examples</a></li>                   
                               <li><a href="<?=$this->config->item('support_url');?>">Help & Support</a></li>
                            </ul>
                        </li>
					 	
			<li class="login">
            <a style="color:#7ae498;
  padding: 5px 7.8px;border: 1px solid #7ae498;color: #7ae498;font-weight: 600;text-shadow: 0 0px 0 #000important;margin-left:10px;margin-right:10px; top:7px; padding: 8px;
  border-radius: 1px;" class="login" href="<?=site_url('dashboard/login/');?>" onclick="login(this)" data-track="header-top" >Dashboard <i class="fa fa-sign-in"></i></a>

                        </li> <!--menu login li end here-->
						
						 <li class="signup">
   <a class="btn border-white btn-signup" style="padding:7.8px 8px 5px 7.8px;padding-left: 8px; font-size:12px; text-align:center;margin-top:7px;" href="<?=$this->config->item('signup_url');?>" onclick="signup(this)" data-track="header-top">Sign Up <i class="fa fa-pencil"></i></a>
</li>

                         </ul>
                </div><!--/.nav-collapse -->
            </div>
        </div><!--.navbar-default-->

		
        <div class="row-fluid">

        <section id="features" class="padded-section">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class="center-heading">
                            <h1>Massive Features</h1>
                            <span class="center-line"></span>
                        </div>
                    </div>                   
                </div><!--center heading row-->
            				
				<div class="row">
				
                    <div class="col-md-4 margin20">
                    <div class="service-box text-center wow animated fadeInLeft animated animated" data-wow-delay="0.2s" style="visibility: visible; animation-delay: 0.2s; animation-name: fadeInLeft;">
                        <i class="fa fa-credit-card"></i>                          
                        <h3>Global Payments</h3>
                        <span class="section-line"></span>
                        <p>
                          Accept payments from your customers in nearly any country, and we never charge for card storage or PCI reporting. 
                           </p>
						   
                    </div><!--service box-->
                </div><!--service col-->
              
                    <div class="col-md-4 margin20">
                    <div class="service-box text-center wow animated fadeInLeft animated animated" data-wow-delay="0.4s" style="visibility: visible; animation-delay: 0.4s; animation-name: fadeInLeft;">
                        <i class="fa fa-cogs"></i>                          
                        <h3>Awesome API's</h3>
                        <span class="section-line"></span>
                        <p>Everpay provides simple API's to be able to integrate payments with your app. Multiple SDK's and languages for easy integration.</p>
			   </div><!--service box-->
					
                </div><!--service col-->
                    <div class="col-md-4 margin20">
                    <div class="service-box text-center wow animated fadeInLeft animated animated" data-wow-delay="0.6s" style="visibility: visible; animation-delay: 0.6s; animation-name: fadeInLeft;">
                        <i class="fa fa-mobile"></i>                          
                        <h3>Mobile Payments</h3>
                        <span class="section-line"></span>
                            <p> All of Everpay's features support multiple mobile devices and opens up endless opportunities to accept mobile payments.  </p>
                   </div><!--service box-->
                </div><!--service col-->
              
                </div><!--services row-->
				
                <div class="divide40"></div>
				
            				
				<div class="row">
				
                    <div class="col-md-4 margin20">
                    <div class="service-box text-center wow animated fadeInLeft animated animated" data-wow-delay="0.2s" style="visibility: visible; animation-delay: 0.2s; animation-name: fadeInLeft;">
                        <i class="fa fa-database"></i>                          
                        <h3>Multiple Connections</h3>
                        <span class="section-line"></span>
                        <p>
                       Everpay works with multiple global payment networks and gateways. Take control of your customers payments today. 
                           </p>
						   
                    </div><!--service box-->
                </div><!--service col-->
              
                    <div class="col-md-4 margin20">
                    <div class="service-box text-center wow animated fadeInLeft animated animated" data-wow-delay="0.4s" style="visibility: visible; animation-delay: 0.4s; animation-name: fadeInLeft;">
                        <i class="fa fa-shield"></i>                          
                        <h3>Fraud Prevention</h3>
                        <span class="section-line"></span>
                        <p>Everpay has developed internal processes and uses innovative partner technologies to provide full spectrum fraud prevention.</p>
	
				   </div><!--service box-->
		                </div><!--service col-->
						
						        <div class="col-md-4 margin20">
                    <div class="service-box text-center wow animated fadeInLeft animated animated" data-wow-delay="0.6s" style="visibility: visible; animation-delay: 0.6s; animation-name: fadeInLeft;">
                        <i class="fa fa-file-pdf-o"></i>                          
                        <h3>Easy Invoicing</h3>
                        <span class="section-line"></span>
                            <p> Everpay allows you to create quotations that convert to invoices, track and get alerts when invoices are paid or viewed.  </p>
                    </div><!--service box-->
                </div><!--service col-->
						
                    <div class="col-md-4 margin20 hidden">
                    <div class="service-box text-center wow animated fadeInLeft animated animated" data-wow-delay="0.6s" style="visibility: visible; animation-delay: 0.6s; animation-name: fadeInLeft;">
                        <i class="fa fa-group"></i>                          
                        <h3>Stellar Support</h3>
                        <span class="section-line"></span>
                            <p> Everpay has first-class, flexible support via live chat, email and phone. We resolve mostly any issue within 24 hours.  </p>
                    </div><!--service box-->
                </div><!--service col-->
              
                </div><!--services row-->
				
            </div>
        </section><!--features-->
				<section class="purchase-sec wow animated fadeInUp animated">
            <div class="container">
                <div class="row">
                    <div class="col-md-8 text-center" style="margin-top:15px;">
                        <span> Easy and customizable payments acceptance</span> 
                    </div>
					
					    <div class="col-md-4">
						  <div class="rev-buttons scroll-to">

                      <a href="<?=$this->config->item('signup_url');?>" class="btn btn-lg border-white">Get Started</a>

                            </div>
						</div>
                </div>
            </div>
        </section><!--cta-->


		<section id="how-it-works" class="how-it-works wow animated fadeInUp animated section-white functions">
		    
            <div class="container how-it-works">
			
                <div class="row">
				
                    <div class="col-md-12">
                        <div class="center-heading">
                            <h1>How It Works</h1>
                            <span class="center-line"></span>
                        </div><!--center-heading-->
                    </div>      
						<div class="divide40"></div>
					   <div class="col-md-12">
			
			<div class="row">
				<div class="col-lg-10 col-lg-offset-1">
					<div class="row js-screens-filter">
						<div class="col-sm-15 i">
							<a href="#tab-claim-account" aria-controls="tab-claim-account" role="tab" data-toggle="tab">
								<span class="icon"><img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/c_scale,w_96/v1444066753/compose_x5og5z.png"></span>
							</a>
							<a href="#tab-claim-account" aria-controls="tab-claim-account" role="tab" data-toggle="tab" class="hoverExtend active">SIGN UP</a>
						</div>
						<div class="col-sm-15 i">
							<a href="#tab-get-verified" aria-controls="tab-get-verified" role="tab" data-toggle="tab">
								<span class="icon"><img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/c_scale,w_96/v1444066757/check_vcy1zm.png"></span>
							</a>
							<a href="#tab-get-verified" aria-controls="tab-get-verified" role="tab" data-toggle="tab" class="hoverExtend">VERIFY ACCOUNT</a>
						</div>
						<div class="col-sm-15 i">
							<a href="#tab-hosted-pages" aria-controls="tab-hosted-pages" role="tab" data-toggle="tab">
								<span class="icon"><img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/c_scale,w_96/v1444066772/dev_uyghgz.png"></span>
							</a>
							<a href="#tab-hosted-pages" aria-controls="tab-hosted-pages" role="tab" data-toggle="tab" class="hoverExtend">START INTEGRATION</a>
						</div>
						<div class="col-sm-15 i">
							<a href="#tab-invoicing" aria-controls="tab-invoicing" role="tab" data-toggle="tab">
								<span class="icon"><img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/c_scale,w_96/v1444066764/contacts_jziodw.png"></span>
							</a>
							<a href="#tab-invoicing" aria-controls="tab-invoicing" role="tab" data-toggle="tab" class="hoverExtend">ADD CUSTOMERS</a>
						</div>
						<div class="col-sm-15 i">
							<a href="#tab-get-paid" aria-controls="tab-get-paid" role="tab" data-toggle="tab">
								<span class="icon"><img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/c_scale,w_96/v1444066794/money_fhsf5n.png"></span>
							</a>
							<a href="#tab-get-paid" aria-controls="tab-get-paid" role="tab" data-toggle="tab" class="hoverExtend">COLLECT PAYMENTS</a>
						</div>
					</div>
					
					<div class="tab-content notebook">
						<!-- Notebook -->
						<img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1442882912/macbook_kafxi2.png" alt="#" width="968" height="510" class="img-responsive">
						
						<!-- Images -->
						<div role="tabpanel" class="tab-pane fade image in active" id="tab-claim-account">
							<img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1444029500/login-screen_rpheky.jpg" alt="#" width="667" height="417" class="img-responsive">
						</div>
						<div role="tabpanel" class="tab-pane fade image" id="tab-get-verified">
							<img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/c_scale,w_667/v1444060732/get_verified_tfxzdr.jpg" alt="#" width="667" height="417" class="img-responsive">
						</div>
						<div role="tabpanel" class="tab-pane fade image" id="tab-hosted-pages">
							<img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/c_scale,w_667/v1444066607/start_integration_u6fu04.jpg" alt="#" width="667" height="417" class="img-responsive">
						</div>
						<div role="tabpanel" class="tab-pane fade image" id="tab-invoicing">
							<img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/c_scale,w_667/v1444060695/add_customer_qhk8vx.jpg" alt="#" width="667" height="417" class="img-responsive">
						</div>
						<div role="tabpanel" class="tab-pane fade image" id="tab-get-paid">
							<img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/c_scale,w_667/v1444060740/collect_payments_xrdsbc.jpg" alt="#" width="667" height="417" class="img-responsive">
						</div>
					</div>
				</div>
			</div>
		</div>
	</section>				
                </div><!--/row-->
				             
		<div class="divide30"></div>
		
			</div><!--container-->
		</section>
   
   <section id="cta-3">
            <div class="container">
                <h1>Looking for an easier way to get paid?</h1>
                <a href="<?=$this->config->item('signup_url');?>" class="btn border-white btn-lg">Get Started</a>
            </div>
        </section><!--cta-->
		
        <section class="fun-fact-wrap parallax" data-stellar-background-ratio="0.5">
            <div class="container">               
                <div class="row">
                    <div class="col-md-3 margin20 facts-in">
                        <h3><i class="fa fa-image"></i><span class="counter">33</span></h3>
                        <h4>App integrations</h4>
                    </div><!--facts in-->
                    <div class="col-md-3 margin20 facts-in">
                        <h3><i class="fa fa-users"></i><span class="counter">666</span></h3>
                        <h4>Happy Customers</h4>
                    </div><!--facts in-->
                    <div class="col-md-3 margin20 facts-in">
                        <h3><i class="fa fa-clock-o"></i><span class="counter">3220</span></h3>
                        <h4>Invoices Sent </h4>
                    </div><!--facts in-->
                    <div class="col-md-3 margin20 facts-in">
                        <h3><i class="fa fa-rss"></i><span class="counter">69</span></h3>
                        <h4>Payment Connections</h4>
                    </div><!--facts in-->
                </div>
            </div>
        </section><!--how it works-->
		
        
		<section class="purchase-sec wow animated fadeInUp animated">
            <div class="container">
                <div class="row">
                    <div class="col-md-12 text-center">
                        <span>Everpay made it easy for me to collect customers payments. - Brian Humphries</span> 
                    </div>
                </div>
            </div>
        </section><!--cta-->
        <section id="price" class="padded-section">
		
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
					
                        <div class="center-heading">
                            <h1>Sign Up For A 14-Day Free Trial</h1>
                            <span class="center-line"></span>
               <p class="lead">
            
            Awesome Pricing with no hidden fees and doesn't break your bank account.
            <a href="" target="_blank" style="text-decoration:none;"></a> 
                        </p>
						<br />

            </div>
      </div><!--row text-center-->       
	    </div><!--container--> 
           
  <div class="row-fluid db-padding-btm db-attached">
  
            <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 wow animated fadeInRight animated">
                <div class="db-wrapper">
                    <div class="db-pricing-eleven db-bk-color-one">
                        <div class="price">
                            <sup>$</sup>9.95
                                <small>per month</small>
                        </div>
                        <div class="type">
                            Start Up Plan
                        </div>
                        <ul>

                            <li><i class="glyphicon glyphicon-print"></i>CREDIT CARD BILLING </li>
                            <li><i class="glyphicon glyphicon-time"></i>INVOICE BILLING </li>
                            <li><i class="glyphicon glyphicon-trash"></i>ECHECK BILLING</li>
                        </ul>
                        <div class="pricing-footer">

                            <a href="<?=$this->config->item('signup_url');?>" class="btn db-button-color-square btn-lg">GET STARTED</a>
                        </div>
                    </div>
                </div>
             </div><!--price col-->
			 
            <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 wow animated fadeInRight animated">
                  <div class="db-wrapper">
                <div class="db-pricing-eleven db-bk-color-two popular">
                    <div class="price">
                        <sup>$</sup>19.95
                                <small>per month</small>
                    </div>
                    <div class="type">
                        STANDARD PLAN
                    </div>
                    <ul>

                        <li><i class="glyphicon glyphicon-print"></i>Virtual Terminal </li>
                        <li><i class="glyphicon glyphicon-time"></i>Multi-Currency</li>
                        <li><i class="glyphicon glyphicon-trash"></i>5 User Accounts</li>
                    </ul>
                    <div class="pricing-footer">

                        <a href="<?=$this->config->item('signup_url');?>" class="btn db-button-color-square btn-lg">GET STARTED</a>
                    </div>
                </div>
                      </div>
  </div><!--price col-->
            <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 wow animated fadeInRight animated">
                  <div class="db-wrapper">
                <div class="db-pricing-eleven db-bk-color-three">
                    <div class="price">
                        <sup>$</sup>39.95
                                <small>per month</small>
                    </div>
                    <div class="type">
                       BUSINESS PLAN
                    </div>
                    <ul>

                        <li><i class="glyphicon glyphicon-print"></i>Advanced Analytics </li>
                        <li><i class="glyphicon glyphicon-time"></i>Loyalty Management</li>
                        <li><i class="glyphicon glyphicon-trash"></i>20 User Accounts</li>
                    </ul>
                    <div class="pricing-footer">

                        <a href="/contact/" class="btn db-button-color-square btn-lg">GET STARTED</a>
                    </div>
                </div>
                      </div>
              </div><!--price col-->
			  
            <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3 wow animated fadeInRight animated">
                  <div class="db-wrapper">
                <div class="db-pricing-eleven db-bk-color-six">
                    <div class="price">
                        <sup></sup>CALL
                                <small>for pricing</small>
                    </div>
                    <div class="type">
                        ENTERPRISE PLAN
                    </div>
                    <ul>

                        <li><i class="glyphicon glyphicon-print"></i>30+ User Accounts </li>
                        <li><i class="glyphicon glyphicon-time"></i>Custom Branding </li>
                        <li><i class="glyphicon glyphicon-trash"></i>Unlimited Support</li>
                    </ul>
                    <div class="pricing-footer">

                        <a href="<?=$this->config->item('signup_url');?>" onclick="signup(this)" data-track="pricing-footer" class="btn db-button-color-square btn-lg">REQUEST QUOTE</a>
                    </div>
                </div>
                      </div>
              </div><!--price col-->
			  
          </div><!--price row-->
		  
<div class="features-list hidden">

<div class="row">

<div class="col-sm-4">
<span><i class="icon-budicon-290"></i></span>
<h5>Any Payment Provider</h5>
<p>Authenticate to any payment network. First Data, TSYS, PayPal, Stripe and <a href="https://everpayinc.com/docs/#gateways" target="_blank">many more.</a></p>
</div>

<div class="col-sm-4">
<span><i class="icon-budicon-606"></i></span>
<h5>Account Freedom</h5>
<p>Upgrade, downgrade or cancel at any time here and our Free plan limited to 10 products so you can try before you buy.</p></div>

<div class="col-sm-4">
<span><i class="icon-budicon-627"></i></span>
<h5>Guaranteed SLA</h5>
<p>Everpay is hosted and secured by the Amazon Web Service cloud for total peace of mind. From vulnerability scans, to intrusion detection and daily backups</p>
</div>

</div>

<div class="row">

<div class="col-sm-4">
<span><i class="icon-budicon-661"></i></span>
<h5>Innovative Features</h5>
<p>StartUp+ and Enterprise plans automatically include authentication to all payment gateway connections hosted by Everpay. Or, you can choose to upgrade.</p></div>

<div class="col-sm-4">
<span><i class="icon-budicon-341"></i></span>
<h5>Reduce Chargebacks</h5>
<p>Your customer's see your company's name on their credit card statement and not Everpay's.</p></div>

<div class="col-sm-4">
<span><i class="icon-budicon-513"></i></span>
<h5>Awesome Support</h5>
<p>Everpay offers great support to our users. Upgrade to premium support and you’ll experience higher ticket prioritization, one hour response time and extended support hours. Learn more <a href="https://everpayinc.com/docs/premium-support" target="_blank">here</a>.</p>
</div>

</div>

<div class="row">
<div class="col-sm-4">
<span><i class="icon-budicon-173"></i></span>
<h5>Fraud Prevention Rules</h5>
<p>Fraud Prevention Rules implement arbitrarily powerful customization of the authentication and authorization pipeline and are included within all pricing plans.<br>Learn more  <a href="https://everpayinc.com/docs/rules" target="_blank">here.</a></p></div>

<div class="col-sm-4">
<span><i class="icon-budicon-499"></i></span>
<h5>Monthly or Yearly Payment Options</h5>
<p>Icing on the cake. Save a few extra bucks when you choose to pay annually.</p>
</div>

<div class="col-sm-4">
<span><i class="icon-budicon-499"></i></span>
<h5>All In One Marketing</h5>
<p>Everpay helps you manage your own promotions, loyalty programs and deals (current or future) easily.</p>
</div>

</div>

</div><!--features-list row-->
  

            </div>

        </div>

    </div>
         
      </div><!--row text-center-->  
	  
	    </div><!--col-md-12-->          
      </div><!--row text-center-->       
	    </div><!--container--> 
		
		  </section><!--pricing-->
		  
		  <section class="faqIntro Wrapper">
		  <div class="row box">
		  <div class="title"><h1>Frequently Asked Questions</h1></div>
		  <ol>
		  <li class="feature col-md-6 col-sm-6 col-6"><span>1</span>
		  <h4>Where is Everpay available?</h4><p>Currently in <strong>North America</strong> and in beta for Mexico &amp; Argentina. In the following months we'll include more countries, and eventually all of the America's.</p></li>
		  <li class="feature col-md-6 col-sm-6 col-6"><span>2</span>
		  <h4>Which payment methods can I accept?</h4>
		  <p>Payment methods vary by country. Check each country's website in order to know the methods your company can accept there.</p></li>
		  <li class="feature col-md-6 col-sm-6 col-6"><span>3</span>
		  <h4>Do I need a merchant account?</h4>
		  <p><strong>Yes.</strong> Everpay operates with your own merchant account, which allows us to provide great features.</p></li>
		  <li class="feature col-md-6 col-sm-6 col-6"><span>4</span>
		  <h4>Does it work on native mobile apps?</h4>
		  <p><strong>Yes.</strong> Everpay works on most major smartphones including IOS, Android, Windows Mobile and Blackberry.</p></li>
		  </ol>
		  <p class="viewMore"><a href="#" class=""></a></p></div>
		  </section>
		  
		  <section id="cta-1"class="hidden">
            <div class="container">
                <h1>Got More Questions?</h1>
                <a href="<?=$this->config->item('contact_url');?>" onclick="signup(this)" data-track="more-questions" class="btn border-white btn-lg">Contact Us Today</a>
            </div>
        </section>
		  
		  <section  class="hidden">
		<div class="container">
	<div class="row">
		<div class="col-md-12">
                        <div class="center-heading">
                            <h1>Testimonials</h1>
                            <span class="center-line"></span>
                        </div>
        </div>     
	</div>
</div>
<div class="carousel-reviews purple-block">
    <div class="container">
        <div class="row">
            <div id="carousel-reviews" class="carousel slide" data-ride="carousel">
            
                <div class="carousel-inner">
                    <div class="item active">
                	    <div class="col-md-4 col-sm-6">
        				    <div class="block-text rel zmin">
						        <a title="" href="#">Hercules</a>
							    <div class="mark">My rating: <span class="rating-input"><span data-value="0" class="glyphicon glyphicon-star"></span><span data-value="1" class="glyphicon glyphicon-star"></span><span data-value="2" class="glyphicon glyphicon-star"></span><span data-value="3" class="glyphicon glyphicon-star"></span><span data-value="4" class="glyphicon glyphicon-star-empty"></span><span data-value="5" class="glyphicon glyphicon-star-empty"></span>  </span></div>
						        <p>Never before has there been a good film portrayal of ancient Greece's favourite myth. So why would Hollywood start now? This latest attempt at bringing the son of Zeus to the big screen is brought to us by X-Men: The last Stand director Brett Ratner. If the name of the director wasn't enough to dissuade ...</p>
							    <ins class="ab zmin sprite sprite-i-triangle block"></ins>
					        </div>
							<div class="person-text rel">
				                <img src="data:image/png;base64,"/>
								<a title="" href="#">Anna</a>
								<i>from Glasgow, Scotland</i>
							</div>
						</div>
            			<div class="col-md-4 col-sm-6 hidden-xs">
						    <div class="block-text rel zmin">
						        <a title="" href="#">The Purge: Anarchy</a>
							    <div class="mark">My rating: <span class="rating-input"><span data-value="0" class="glyphicon glyphicon-star"></span><span data-value="1" class="glyphicon glyphicon-star"></span><span data-value="2" class="glyphicon glyphicon-star-empty"></span><span data-value="3" class="glyphicon glyphicon-star-empty"></span><span data-value="4" class="glyphicon glyphicon-star-empty"></span><span data-value="5" class="glyphicon glyphicon-star-empty"></span>  </span></div>
        						<p>The 2013 movie "The Purge" left a bad taste in all of our mouths as nothing more than a pseudo-slasher with a hamfisted plot, poor pacing, and a desperate attempt at "horror." Upon seeing the first trailer for "The Purge: Anarchy," my first and most immediate thought was "we really don't need another one of these."  </p>
					            <ins class="ab zmin sprite sprite-i-triangle block"></ins>
				            </div>
							<div class="person-text rel">
				                <img src="data:image/png;base64,"/>
						        <a title="" href="#">Ella Mentree</a>
								<i>United States</i>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 hidden-sm hidden-xs">
							<div class="block-text rel zmin">
								<a title="" href="#">Planes: Fire & Rescue</a>
								<div class="mark">My rating: <span class="rating-input"><span data-value="0" class="glyphicon glyphicon-star"></span><span data-value="1" class="glyphicon glyphicon-star"></span><span data-value="2" class="glyphicon glyphicon-star"></span><span data-value="3" class="glyphicon glyphicon-star"></span><span data-value="4" class="glyphicon glyphicon-star"></span><span data-value="5" class="glyphicon glyphicon-star"></span>  </span></div>
    							<p>What a funny and entertaining film! I did not know what to expect, this is the fourth film in this vehicle's universe with the two Cars movies and then the first Planes movie. I was wondering if maybe Disney pushed it a little bit. However, Planes: Fire and Rescue is an entertaining film that is a fantastic sequel in this magical franchise. </p>
								<ins class="ab zmin sprite sprite-i-triangle block"></ins>
							</div>
							<div class="person-text rel">
								<img src="data:image/png;base64,"/>
								<a title="" href="#">Rannynm</a>
								<i>Indonesia</i>
							</div>
						</div>
                    </div>
                    <div class="item">
                        <div class="col-md-4 col-sm-6">
        				    <div class="block-text rel zmin">
						        <a title="" href="#">Hercules</a>
							    <div class="mark">My rating: <span class="rating-input"><span data-value="0" class="glyphicon glyphicon-star"></span><span data-value="1" class="glyphicon glyphicon-star"></span><span data-value="2" class="glyphicon glyphicon-star"></span><span data-value="3" class="glyphicon glyphicon-star"></span><span data-value="4" class="glyphicon glyphicon-star-empty"></span><span data-value="5" class="glyphicon glyphicon-star-empty"></span>  </span></div>
						        <p>Never before has there been a good film portrayal of ancient Greece's favourite myth. So why would Hollywood start now? This latest attempt at bringing the son of Zeus to the big screen is brought to us by X-Men: The last Stand director Brett Ratner. If the name of the director wasn't enough to dissuade ...</p>
							    <ins class="ab zmin sprite sprite-i-triangle block"></ins>
					        </div>
							<div class="person-text rel">
								<img alt="" src="https://myinstantcms.ru/images/img13.png">
								<a title="" href="#">Anna</a>
								<i>from Glasgow, Scotland</i>
							</div>
						</div>
            			<div class="col-md-4 col-sm-6 hidden-xs">
						    <div class="block-text rel zmin">
						        <a title="" href="#">The Purge: Anarchy</a>
							    <div class="mark">My rating: <span class="rating-input"><span data-value="0" class="glyphicon glyphicon-star"></span><span data-value="1" class="glyphicon glyphicon-star"></span><span data-value="2" class="glyphicon glyphicon-star-empty"></span><span data-value="3" class="glyphicon glyphicon-star-empty"></span><span data-value="4" class="glyphicon glyphicon-star-empty"></span><span data-value="5" class="glyphicon glyphicon-star-empty"></span>  </span></div>
        						<p>The 2013 movie "The Purge" left a bad taste in all of our mouths as nothing more than a pseudo-slasher with a hamfisted plot, poor pacing, and a desperate attempt at "horror." Upon seeing the first trailer for "The Purge: Anarchy," my first and most immediate thought was "we really don't need another one of these."  </p>
					            <ins class="ab zmin sprite sprite-i-triangle block"></ins>
				            </div>
							<div class="person-text rel">
								<img alt="" src="https://myinstantcms.ru/images/img14.png">
						        <a title="" href="#">Ella Mentree</a>
								<i>United States</i>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 hidden-sm hidden-xs">
							<div class="block-text rel zmin">
								<a title="" href="#">Planes: Fire & Rescue</a>
								<div class="mark">My rating: <span class="rating-input"><span data-value="0" class="glyphicon glyphicon-star"></span><span data-value="1" class="glyphicon glyphicon-star"></span><span data-value="2" class="glyphicon glyphicon-star"></span><span data-value="3" class="glyphicon glyphicon-star"></span><span data-value="4" class="glyphicon glyphicon-star"></span><span data-value="5" class="glyphicon glyphicon-star"></span>  </span></div>
    							<p>What a funny and entertaining film! I did not know what to expect, this is the fourth film in this vehicle's universe with the two Cars movies and then the first Planes movie. I was wondering if maybe Disney pushed it a little bit. However, Planes: Fire and Rescue is an entertaining film that is a fantastic sequel in this magical franchise. </p>
								<ins class="ab zmin sprite sprite-i-triangle block"></ins>
							</div>
							<div class="person-text rel">
								<img alt="" src="https://myinstantcms.ru/images/img15.png">
								<a title="" href="#">Rannynm</a>
								<i>Indonesia</i>
							</div>
						</div>
                    </div>
                    <div class="item">
                        <div class="col-md-4 col-sm-6">
        				    <div class="block-text rel zmin">
						        <a title="" href="#">Hercules</a>
							    <div class="mark">My rating: <span class="rating-input"><span data-value="0" class="glyphicon glyphicon-star"></span><span data-value="1" class="glyphicon glyphicon-star"></span><span data-value="2" class="glyphicon glyphicon-star"></span><span data-value="3" class="glyphicon glyphicon-star"></span><span data-value="4" class="glyphicon glyphicon-star-empty"></span><span data-value="5" class="glyphicon glyphicon-star-empty"></span>  </span></div>
						        <p>Never before has there been a good film portrayal of ancient Greece's favourite myth. So why would Hollywood start now? This latest attempt at bringing the son of Zeus to the big screen is brought to us by X-Men: The last Stand director Brett Ratner. If the name of the director wasn't enough to dissuade ...</p>
							    <ins class="ab zmin sprite sprite-i-triangle block"></ins>
					        </div>
							<div class="person-text rel">
								<img alt="" src="https://myinstantcms.ru/images/img13.png">
								<a title="" href="#">Anna</a>
								<i>from Glasgow, Scotland</i>
							</div>
						</div>
            			<div class="col-md-4 col-sm-6 hidden-xs">
						    <div class="block-text rel zmin">
						        <a title="" href="#">The Purge: Anarchy</a>
							    <div class="mark">My rating: <span class="rating-input"><span data-value="0" class="glyphicon glyphicon-star"></span><span data-value="1" class="glyphicon glyphicon-star"></span><span data-value="2" class="glyphicon glyphicon-star-empty"></span><span data-value="3" class="glyphicon glyphicon-star-empty"></span><span data-value="4" class="glyphicon glyphicon-star-empty"></span><span data-value="5" class="glyphicon glyphicon-star-empty"></span>  </span></div>
        						<p>The 2013 movie "The Purge" left a bad taste in all of our mouths as nothing more than a pseudo-slasher with a hamfisted plot, poor pacing, and a desperate attempt at "horror." Upon seeing the first trailer for "The Purge: Anarchy," my first and most immediate thought was "we really don't need another one of these."  </p>
					            <ins class="ab zmin sprite sprite-i-triangle block"></ins>
				            </div>
							<div class="person-text rel">
								<img alt="" src="https://myinstantcms.ru/images/img14.png">
						        <a title="" href="#">Ella Mentree</a>
								<i>United States</i>
							</div>
						</div>
						<div class="col-md-4 col-sm-6 hidden-sm hidden-xs">
							<div class="block-text rel zmin">
								<a title="" href="#">Planes: Fire & Rescue</a>
								<div class="mark">My rating: <span class="rating-input"><span data-value="0" class="glyphicon glyphicon-star"></span><span data-value="1" class="glyphicon glyphicon-star"></span><span data-value="2" class="glyphicon glyphicon-star"></span><span data-value="3" class="glyphicon glyphicon-star"></span><span data-value="4" class="glyphicon glyphicon-star"></span><span data-value="5" class="glyphicon glyphicon-star"></span>  </span></div>
    							<p>What a funny and entertaining film! I did not know what to expect, this is the fourth film in this vehicle's universe with the two Cars movies and then the first Planes movie. I was wondering if maybe Disney pushed it a little bit. However, Planes: Fire and Rescue is an entertaining film that is a fantastic sequel in this magical franchise. </p>
								<ins class="ab zmin sprite sprite-i-triangle block"></ins>
							</div>
							<div class="person-text rel">
								<img alt="" src="https://myinstantcms.ru/images/img15.png">
								<a title="" href="#">Rannynm</a>
								<i>Indonesia</i>
							</div>
						</div>
                    </div>                    
                </div>
                <a class="left carousel-control" href="#carousel-reviews" role="button" data-slide="prev">
                    <span class="glyphicon glyphicon-chevron-left"></span>
                </a>
                <a class="right carousel-control" href="#carousel-reviews" role="button" data-slide="next">
                    <span class="glyphicon glyphicon-chevron-right"></span>
                </a>
            </div>
        </div>
		
		
        </section><!--customers-->
		
<hr>

        <section id="our-team" class="our-team padded-section">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class="center-heading">
                            <h1>The Team</h1>
                            <span class="center-line"></span>
							<h3> Our mission is to provide the most simplest way to collect customer payments.</h3>
                        </div>
                    </div>            
<br />					
                </div><!--center heading row--> 
                <div class="row">
                    <div class="col-sm-3">
                        <div class="person">
                            <img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1442882774/80x80_dy3njb.jpg" class="img-responsive img-circle" width="68" alt="">
                            <div class="person-desc">
                                <h4>Richard Rowe</h4>
                                <em>CEO/ Founder</em>
                            </div>
                        </div>
                    </div><!--team-col-->
                     <div class="col-sm-3">
                        <div class="person">
                            <img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1442882789/team-3_cfknmj.jpg" class="img-responsive img-circle" width="68" alt="">
                            <div class="person-desc">
                                <h4>Ibra Diop</h4>
                                <em>COO</em>
                            </div>
                        </div>
                    </div><!--team-col-->
                     <div class="col-sm-3">
                        <div class="person">
                            <img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1442882789/68x68-1_lfylyq.jpg" class="img-responsive img-circle" width="68" alt="">
                            <div class="person-desc">
                                <h4>Kevin Chapman</h4>
                                <em>CFO</em>
                            </div>
                        </div>
                    </div><!--team-col-->
					
					
                     <div class="col-sm-3">
                        <div class="person">
                            <img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1442882789/team-2_gtd22x.jpg" class="img-responsive img-circle" width="68" alt="">
                            <div class="person-desc">
                                <h4>Jitesh Tukadiya</h4>
                                <em>Lead Developer</em>
                            </div>
                        </div>
                    </div><!--team-col-->
					
                </div>
				<p> Between all the key people involved in creating Everpay we have around 75+ years of experience in the payments, sales and internet technologies. With all that knowledge power we know one thing for sure is, this stuff ain't easy! We started Everpay to simplify this complex process and thus formulating our mission actually became very clear:

</p>
            </div>
        </section><!--our team-->
		
        <section id="cta-3">
            <div class="container">
			
                <div class="row">
                    <div class="col-md-6 password">
					<h4><a href="https://everpayinc.com/dashboard/login">
					<i class="fa fa-key fa-3x pull-left"></i>Forgot your password? We'll help you reset it.
					</a></h4>
                    </div><!--col-md-6-->
					
			  <div class="col-md-6 noaccount">
			  <h4><a href="/support/"> 
			  <i class="fa fa-question fa-3x pull-left"></i>Not using Everpay yet? We will still answer your questions!
			  </a></h4>
                    </div><!--col-md-6-->
					
                </div><!--row-->
		          </div>
        </section><!--cta-->
		
		
        <section id="clients-carousel">
		
            <div class="container">
			
                <div class="row">
                    <div class="col-md-12">
                        <div class="center-heading">
                            <h1>Payment Connections</h1>
                            <span class="center-line"></span>
                          <p class="lead">
                            We add new processors and functionalities frequently; please contact us if you need particular functions or the processor you would like to be added. 
                            </p>
                        </div> <!--center heading-->
                    </div><!--col-md-12-->
					
                </div><!--row-->
                <div id="clients-slider"> 
		
			<div class="clearfix" style="height: 20px;"></div> 
			
                    <div class="item"><a href="#"><img src="<?=branded_include('onepage/img/processors/chase.png');?>" alt=""></a></div>
                    <div class="item"><a href="#"><img src="<?=branded_include('onepage/img/processors/paypal.png');?>" alt=""></a></div>
                    <div class="item"><a href="#"><img src="<?=branded_include('onepage/img/processors/tsys.png');?>" alt=""></a></div>
                    <div class="item"><a href="#"><img src="<?=branded_include('onepage/img/processors/stripe.png');?>" alt=""></a></div>
   					<div class="item"><a href="#"><img src="<?=branded_include('onepage/img/processors/global.png');?>" alt=""></a></div>
                    <div class="item"><a href="#"><img src="<?=branded_include('onepage/img/processors/intuit.png');?>" alt=""></a></div>
                    <div class="item"><a href="#"><img src="<?=branded_include('onepage/img/processors/firstdata.png');?>" alt=""></a></div>
                    <div class="item"><a href="#"><img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1442890489/logo-worldpay_m6cpsi.gif" alt=""></a></div>
                    <div class="item"><a href="#"><img src="https://res.cloudinary.com/lmj6rf6tz/image/upload/v1442890575/rbs_pjz0vq.png" alt=""></a></div>
                    <div class="item"><a href="#"><img src="<?=branded_include('onepage/img/processors/elavon.png');?>" alt=""></a></div>
                    
				</div>
				

            </div><!--container-->
        </section><!--clients carousel end-->

        <section id="cta-1">
            <div class="container">
                <h1>Already Have A Merchant Account?</h1>
                <a href="<?=$this->config->item('signup_url');?>" onclick="signup(this)" data-track="cta-bottom"  class="btn border-white btn-lg">Connect It Now</a>
            </div>
        </section><!--cta-->

		</div><!--row-fluid-->

		<script src="https://maps.googleapis.com/maps/api/js?sensor=false"></script>
		<script src="https://maps.googleapis.com/maps-api-v3/api/js/22/7/main.js"></script>
<?=$this->load->view(branded_view('common/footer'));?>