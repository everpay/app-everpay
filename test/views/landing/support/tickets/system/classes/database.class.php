		...
		switch ($bt_db_type) {
			...
			case 'sqlite':
				...
				if (array_search('sqlite', parent::getAvailableDrivers()) === FALSE) {
					$error->create(array('type' => 'sql_type_unsupported', 'message' => 'Unable to find the PDO SQLite database driver, which is required with the current database settings.'));
				}
						
				try {
					$connection = parent::__construct('sqlite:dbname=' . $bt_db_name);
				}
				catch (\PDOException $e) {
					$error->create(array('type' => 'sql_connect_error', 'message' => $e->getMessage()));
				}
