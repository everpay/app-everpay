...
class mailer {
	...
	public function queue_email($email_array) {
		...
		$id = $queue->add(array('data' => $email_array, 'type' => 'email'));
	...
	public function send_email($array) {
		...
		try {
			...
			//cc users
			if (isset($array['cc']) && is_array($array['cc'])) {
				foreach($array['cc'] as $cc_item) {
					if (isset($cc_item['to_name'])) {
						$this->phpmailer->AddCC($cc_item['to'], $cc_item['to_name']);
					}
					else {
						$this->phpmailer->AddCC($cc_item['to'], '');
					}
				}
			}			
			...
			if (isset($array['to']) && is_array($array['to'])) {
				$array['event_description'] = 'Email sent to "' . safe_output($array['to']['to']) . '" from "' . $this->phpmailer->From . '" with subject "'.safe_output($array['subject']).'"';				
			}
			else {
				$array['event_description'] = 'Email sent from "' . $this->phpmailer->From . '" with subject "'.safe_output($array['subject']).'"';
			}
