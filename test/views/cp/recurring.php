	...
	<tbody>
		...
		<tr>
			...
			<td style="width: 75%"><?=$plan['name'];?><span id="plan_updater_link"> (<a href="#">change plan</a>)</span>
				<span id="plan_updater">
					...
					<? } ?>
			</select>&nbsp;<input type="submit" class="btn btn-success btn-lg btn-block" name="submit2" value="Update" />
					</form>
				</span>
			</td>
		</tr>
		...
		<tr>
			...
			<tr>
				...
				<td>****<?=$card_last_four;?> (<a href="<?=site_url('transactions/update_cc/' . $id);?>">update credit card info</a>)</td>
			</tr>
			<? } ?>
		<? } else { ?>
		<tr>
		...
		<tr>
			...
			<td><a href="<?=site_url('settings/edit_gateway/' . $gateway['gateway_id']);?>"class="btn btn-primary btn-xs" ><span class="fa fa-pencil"></span><?=$gateway['alias'];?></a></td>
		</tr>
	</tbody>
	<? if (isset($customer)) { ?>
	<thead>
