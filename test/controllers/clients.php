...
class Clients extends Controller {
	...
	*/
	function index()
	{
		...
		$this->navigation->SidebarButton('<span class="section__btn section__btn--round"><i class="fa fa-plus"></i> User</span>','clients/create');
...
/*
	...
	*/
	function create () {
		...
		// sidebar
		$this->navigation->SidebarButton('<span class="section__btn section__btn--round"><i class="fa fa-plus"></i> Import</span>','clients/import');
	...
	*/
	function post ($action, $id = false) {
		...
		if ($this->input->post('client_type') == '1' and $this->user->Get('client_type') == '1') {
			$this->notices->SetError('You do not have permission to create Service Provider accounts.');
			$error = true;
		}
		elseif ($this->user->Get('client_type') == '3') {
			$this->notices->SetError('You do not have user account creation privileges.');
			$error = true;
		}
		...
		if ($this->input->post('first_name') == '') {
			$this->notices->SetError('First Name is required.');
			$error = true;
		}
		if ($this->input->post('last_name') == '') {
			$this->notices->SetError('Last Name is required.');
			$error = true;
		}
		...
		if ($action == 'new') {
			...
			if (is_array($clients)) {
				$this->notices->SetError('The username is not unique.');
				$error = true;
			}
		}
		if (isset($error)) {
