...
class Invoices extends REST_Controller {
	...
	public function show_get()
	{
		...
		{
			$this->response(array('status' => false, 'error_message' => 'No unique_id (or id) was provided.'), 400);
		}
		
		if ($this->get('unique_id')) {
		    $unique_id = $this->get('unique_id');
		} else {
		    $unique_id = $this->invoice_m->getIdByUniqueId($this->get('id'));
		}
	...
	public function new_post()
	{
			...
			if ($tasks = $this->project_task_m->get_tasks_by_project($project->id))
			{
				foreach ($tasks->result() as $task)
				{
					$items[] = array(
						...
						'rate' => $task->rate,
						'tax_id' => 0,
						'total' => round($task->hours * $task->rate, 2),
					);
				}
			}
		...
		if ($post_items = $this->input->post('items'))
		{
			...
			{
				if ( ! isset($item['name'], $item['rate'], $item['rate'], $item['rate']))
				{
					$this->response(array('status' => false, 'error_message' => 'Item ['.$i.'] is missing some items.'), 400);
				}
