...
class Client implements ClientInterface
{
    public function getClient($clientId, $clientSecret = null, $redirectUri = null, $grantType = null)
    {
        ...
        if ( ! is_null($redirectUri) && is_null($clientSecret)) {
            $stmt = $db->prepare('SELECT oauth_clients.id, oauth_clients.secret, oauth_client_endpoints.redirect_uri, oauth_clients.name, oauth_clients.homepage FROM oauth_clients LEFT JOIN oauth_client_endpoints ON oauth_client_endpoints.client_id = oauth_clients.id WHERE oauth_clients.id = :clientId AND oauth_client_endpoints.redirect_uri = :redirectUri');
            $stmt->bindValue(':redirectUri', $redirectUri);
        }

        elseif ( ! is_null($clientSecret) && is_null($redirectUri)) {
            $stmt = $db->prepare('SELECT oauth_clients.id, oauth_clients.secret, oauth_clients.name, oauth_clients.name.homepage FROM oauth_clients  WHERE oauth_clients.id = :clientId AND oauth_clients.secret = :clientSecret');
        ...
        return array(
            ...
            'homepage'      =>  (isset($row->homepage)) ? $row->homepage : null,
            'name'          =>  $row->name
        );
    }
}