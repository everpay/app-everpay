	...
	function index()
	{
		...
		$columns = array(
						...
						array(
							...
							'filter' => 'amount'),
						array(
							'name' => '<i class="icon-budicon-292 mr-3"></i> Customer',
						...
						array(
							...
							'sort_column' => 'status',
							'type' => 'select',
							'options' => array('1' => '<span class="label label-success">ok&nbsp;</span>', '2' => 'refunded', '0' => '<span class="label label-danger">failed&nbsp;</span>', '3' => 'failed-repeat'),
							'width' => '4%',
							'filter' => 'status'),
						...
						array(
							'name' => '<i class="icon-budicon-330 ml-2"></i>',
							'width' => '5%'
							)
					);
...
// echo '<pre/>';print_r($this->dataset->data);die();
	...
	*/
	...
	function create2() {
		...
		$this->load->view(branded_view('cp/new_transaction2.php'), $data);
		return TRUE;
	}
