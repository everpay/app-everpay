...
class Navigation extends Model {
    ...
    */
    function Output () {
    	...
    	while (list(,$link) = each($this->elements)) {
    		...
    		if (isset($this->children[$link['link']])) {
				$return .= '<li ' . $this->ArrayToClass($classes) . '>
                <a class="nav-link nav-dropdown-toggle" href="#">' . $link['name'] . '</a>';
	    			$return .= '<ul class="nav-dropdown-items">';

    			while (list(,$child) = each($this->children[$link['link']])) {
    				$displaylink = ($child['external'] == false) ? site_url($child['link']) : $child['link'];
    				$return .= '<li class="nav-item" ><a class="nav-link" href="' . $displaylink . '"> ' . $child['name'] . '</a></li>';
    			}
    ...
    * @return boolean TRUE upon success.
    */
    function SidebarButton ($text, $link, $class = 'main-panel__switch__button', $external = false) {
    ...
    function GetSidebar () {
    	...
    	if (is_array($this->sidebar_notes)) {
	    	foreach ($this->sidebar_notes as $note) {
	    		$return .= '<p class="alert alert-info fade in">' . $note['text'] . '</p>';
	    	}
	    }
